...
(function () {
  ...
  var WA_MANAGER = 'https://wa.me/48502970935';
        ...
        .then(function (res) {
          if (res && res.ok) {
            ...
            try {
              ...
              window.dataLayer.push(Object.assign({ event: 'form_success' }, evtData));
              /* GA4 (gtag) direct — lead conversion */
              if (typeof window.gtag === 'function') {
                window.gtag('event', 'generate_lead', evtData);
              }
            } catch (e) {}
            onSuccess();
          } else {
  ...
  if (blogFilter) {
    var chips = blogFilter.querySelectorAll('.chip');
    var bposts = document.querySelectorAll('#blogGrid .post');
    blogFilter.addEventListener('click', function (e) {
      var b = e.target.closest('.chip');
      if (!b) return;
      chips.forEach(function (c) { c.classList.toggle('is-active', c === b); });
      var fl = b.getAttribute('data-filter');
      bposts.forEach(function (p) {
        var tags = ' ' + (p.getAttribute('data-tags') || '') + ' ';
        p.style.display = (fl === 'all' || tags.indexOf(' ' + fl + ' ') > -1) ? '' : 'none';
      });
    });
  }
